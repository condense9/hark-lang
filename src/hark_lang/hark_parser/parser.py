...
class HarkParseError(UserResolvableError):
    """Parse error"""
...
def tl_parse(filename: str, text: str, debug_lex=False):
    ...
    parser = HarkParser(filename, text)
    lexer = HarkLexer(filename, text)
    if debug_lex:
